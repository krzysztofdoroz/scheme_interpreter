...
object parser_test {
  ...
  val s_4 = "(inc 2)"                             //> s_4  : String = (inc 2)
  ...
  val in_4 = Parser.tokenize(s_4)                 //> in_4  : List[String] = List("", (, inc, 2, ))
  
  
  Parser.parse(in)                                //> res0: List[Any] = List(Define(), VarName(x), IntConst(2))
  
  Parser.parse(in_2)                              //> res1: List[Any] = List(Define(), VarName(inc), List(Lambda(), List(VarName(x
                                                  //| )), List(ProcName(+), VarName(x), IntConst(1))))
                   
  Parser.parse(in_3)                              //> res2: List[Any] = List(VarName(x))
  
  Parser.parse(in_4 )                             //> res3: List[Any] = List(VarName(inc), IntConst(2))
